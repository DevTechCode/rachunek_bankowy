...
export class VatDetector {
   ...
   */
  public detectVatInfo(type: string, desc: ParsedDescription, statementCurrency: Currency): VatInfo | undefined {
   ...
   */
  public detectSplitPayment(type: string, amountMinor: bigint, vatInfo?: VatInfo): boolean {
    if (!vatInfo?.vatAmount) return false;
   ...
   */
  private extractVatAmount(desc: ParsedDescription, statementCurrency: Currency): Money | undefined {
    const raw = desc.getFirst("kwota vat");
    if (!raw) return undefined;
    const m = raw.match(/^([+-]?[0-9\s.,]+)\s*([A-Z]{3})?$/);
    if (!m) return undefined;
    const amount = m[1];
    const currency = (m[2] as Currency | undefined) ?? statementCurrency;
   ...
   * @param desc - opis
   */
  private extractInvoiceOrPeriod(desc: ParsedDescription): {
    invoiceNumber?: string;
    paymentPeriodFromInvoiceField?: string;
  } {
   ...
   * @param desc - opis
   */
  private extractTaxForm(type: string, desc: ParsedDescription): string | undefined {
    ...
    if (/podatkow/i.test(type)) return undefined;
    return undefined;
  }
   ...
   * @param desc - opis
   */
  private extractPaymentPeriod(desc: ParsedDescription): string | undefined {
