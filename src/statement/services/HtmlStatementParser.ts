 ...
 */
export class HtmlStatementParser implements StatementParser {
  private readonly descriptionParser = new DescriptionParser();
  private readonly vatDetector = new VatDetector();
  private readonly counterpartyExtractor = new CounterpartyExtractor();
  private readonly categoryEngine = new CategoryRuleEngine();
   ...
   * @param $ - cheerio root
   */
  private findBestTable($: cheerio.CheerioAPI): Element | undefined {
    const tables = $("table").toArray();
    for (const t of tables) {
      const { headerMap } = this.buildHeaderMap($, t);
      if (headerMap) return t;
    }
    return undefined;
  }
  ...
  private buildHeaderMap(
    $: cheerio.CheerioAPI,
    table: Element
  ): { headerMap?: Map<string, number>; headerRowIndex: number } {
  ...
  private parseRow(
    ...
    const valueDate = this.parseDateCell(
      this.getCell($, cells, headerMap, ["datawaluty", "datarealizacji", "execdate", "valuedate"]) ??
    ...
    const currency = (currencyRaw.trim().toUpperCase() || this.detectCurrency(amountRaw) || "PLN") as Currency;
   ...
   */
  private anyHeader(map: Map<string, number>, keys: string[]): boolean {
   ...
   */
  private parseDateCell(raw?: string): Date {
    const s = (raw ?? "").trim();
    // YYYY-MM-DD
   ...
   */
  private detectCurrency(raw: string): string | undefined {
   ...
   */
  private stripCurrency(raw: string): string {
