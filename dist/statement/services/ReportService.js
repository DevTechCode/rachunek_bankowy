...
export class ReportService {
    ...
    monthlySummary(transactions) {
        ...
        for (const [month, txs] of byMonth.entries()) {
            ...
            for (const t of txs) {
                ...
                if (t.amount.minor < 0n)
                    expense = expense.add(t.amount.abs());
            }
     ...
     */
    topCounterparties(transactions, limit = 10) {
        const byFp = new Map();
        for (const t of transactions) {
        ...
        res.sort((a, b) => {
            if (a.totalAbs.minor === b.totalAbs.minor)
